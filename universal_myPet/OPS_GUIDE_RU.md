...
При интерактивном запуске скрипт сам проверяет прошлый незавершенный запуск и предлагает:
- продолжить по сохраненным checkpoints;
- сбросить checkpoints и начать заново;
- выйти без запуска.

После успешного полного прогона checkpoints очищаются автоматически.  
...
- для такой строки checkpoint все равно сохраняется (чтобы не создавать дубль при повторном запуске);
